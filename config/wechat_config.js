const Wechat = require('../lib/wechat_api');
const WechatOAth = require('../lib/oauth');
...
const wechatConfig = {
    wechat: {
        ...
        async saveUserAppid(appid) {
            const sql = `insert into user_appid (appid) value ('${appid}')`;
            pool.query(sql, (err) => {
                if (err) console.error('saveUserAppid fails!');
            })
        },
        async searchUserAppid() {
            const sql = 'select * from user_appid';
            return new Promise(function (resolve, reject) {
                pool.query(sql, (err, data) => {
                    if (err) reject(err);
                    resolve(data);
        ...
        async delUserAppid(appid) {
            const sql = `delete from user_appid where appid='${appid}'`;
            pool.query(sql, err => {
                if (err) console.error('delUserAppid fails!');
            })
        }
    }
};
